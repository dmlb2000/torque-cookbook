...
nfs_export '/home' do
  ...
  writeable true
  sync true
  options %w(no_root_squash insecure)
end
...
if Chef::Config[:solo]
  ...
  cnodes = search(:node, 'roles:torque-compute AND '\
...
torque_docdir = case node['platform_family']
                when 'fedora'
...
if Chef::Config[:solo]
  ...
  cnodes = search(:node, 'roles:torque-clients AND '\
                   "chef_environment:#{node.environment}"
                 )
end
